#!/usr/bin/env python3
import os, json, sqlite3, logging, hashlib, threading, re
...
@login_required
def api_status():
    return jsonify({"bot_running":bot_manager.running, "active_channels":list(active_channels.keys())})
...
class BotManager:
    ...
    def get_webhook_info(self):
        if not self.bot:
            return {}
        try: return self.bot.get_webhook_info().to_dict()
        except: return {}
    ...
    def cmd_start(self,update:Update,context):
        update.message.reply_text("Bot running via Webhook!")

    def cmd_setapi(self,update:Update,context):
        update.message.reply_text("Send API_ID and API_HASH separated by space.")
